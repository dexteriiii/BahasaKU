/* ==================================================================
   ...
   dan level3.html sehingga datanya konsisten di seluruh aplikasi.
   ================================================================== */

const BK_STORAGE_KEY = 'bahasaku_progress';

const BK_BADGES = [
    { id: 'first_step',    name: 'First Step',    icon: '🥇', desc: 'Selesaikan Level 1' },
    { id: 'flawless',      name: 'Flawless',      icon: '💎', desc: 'Selesaikan satu level tanpa kesalahan sama sekali' },
    { id: 'sharp_ear',     name: 'Sharp Ear',     icon: '🎧', desc: 'Nilai sempurna di Level 3 (Listening)' },
    { id: 'trio_complete', name: 'Trio Complete', icon: '👑', desc: 'Selesaikan Level 1, 2, dan 3' }
];

function bkGetData() {
    try {
        const raw = localStorage.getItem(BK_STORAGE_KEY);
        if (!raw) return { level1: null, level2: null, level3: null, badges: [] };
        ...
        return { level1: null, level2: null, level3: null, badges: [] };
    }
}
...
function bkRecordAnswer(level, questionId, isCorrect) {
    ...
    if (!data[level]) data[level] = { score: null, mistakes: [], completed: false };
...
function bkCheckBadges(data) {
    ...
    if (data.level3 && data.level3.completed && data.level3.score &&
        data.level3.score.correct === data.level3.score.total) {
        unlocked.add('sharp_ear');
    }

    if (data.level1 && data.level1.completed &&
        data.level2 && data.level2.completed &&
        data.level3 && data.level3.completed) {
        unlocked.add('trio_complete');
    }
...
function bkResetProgress() {
    if (confirm('Hapus semua skor, kesalahan, dan lencana? Tindakan ini tidak bisa dibatalkan.')) {
...
function bkRenderDashboard() {
    const data = bkGetData();
    const levels = [
        { key: 'level1', label: 'Level 1 — Basic Vocabulary', color: '#2D9CDB', file: 'index.html' },
        { key: 'level2', label: 'Level 2 — Grammar',          color: '#9b51e0', file: 'level2.html' },
        { key: 'level3', label: 'Level 3 — Listening',        color: '#0D9488', file: 'level3.html' }
    ];

    // --- Score board ---
    const scoreBoard = document.getElementById('dashboard-scoreboard');
    if (scoreBoard) {
        scoreBoard.innerHTML = levels.map(lv => {
            const d = data[lv.key];
            const scoreText = d && d.score ? `${d.score.correct} / ${d.score.total}` : 'Belum dimainkan';
            const bestText = d && d.score ? `Rekor terbaik: ${d.score.best} / ${d.score.total}` : '';
    ...
    if (retryContainer) {
        const withMistakes = levels.filter(lv => (data[lv.key] && data[lv.key].mistakes && data[lv.key].mistakes.length > 0));
        if (withMistakes.length === 0) {
            ...
            retryContainer.innerHTML = withMistakes.map(lv => {
